...
example:   ℕ.one.sub ℕ.one  = ℕ.zero := rfl
example: ℕ.three.sub ℕ.one  = ℕ.two  := rfl
example:   ℕ.two.sub ℕ.four = ℕ.zero := rfl

-- Let's instantiate the `Sub` typeclass with this!
instance: Sub ℕ where sub := ℕ.sub

-- Now we can use `x - y` notation
example:   ℕ.one - ℕ.one  = ℕ.zero := rfl
example: ℕ.three - ℕ.one  = ℕ.two  := rfl
example:   ℕ.two - ℕ.four = ℕ.zero := rfl
